...
def refresh(*,reason='MANUAL_REFRESH',execution_context='INTERNAL_PROBE',run_id=None,force=False):
 ...
 run_id=run_id or uuid.uuid4().hex;built=nowz();daily,dp,sp,snap,session,provider,ppfile,pp,ppmap=source_inputs();rows,closed=db_open();base=base_path();truth=connect_truth();pdb=sqlite3.connect(base/'packets/holding_state_packets.sqlite');rdb=sqlite3.connect(base/'receipts/packet_load_validation_receipts.sqlite')
 packet_index={};receipt_index={};results=[]
 for row in rows:
  t=row['ticker'];pkt=build_packet(row,dp.get(t),ppmap.get(t),bar(provider,t,session),session,built);rec=receipt(pkt,row,session,built);verify_packet(pkt);verify_receipt(rec,pkt)
 ...
 return {'status':'PASS' if len(results)==4 and all(x['receipt_usability']=='USABLE' for x in results) else 'DATA_INCOMPLETE','run_id':run_id,'session':session,'execution_context':execution_context,'reason':reason,'manifest_path':str(mp),'manifest_pointer':str(pointer_path()),'manifest_digest':manifest['manifest_digest'],'open_set':[x['ticker'] for x in results],'packets':results,'lasr_closed_evidence':closed,'bypass_artifact':str(bypass),'send_boundary':sb,'truth_db':str(truth_db()),'source_paths':{'daily':str(daily_path()),'snapshot':str(sp) if sp else None,'profit_protection':str(ppfile) if ppfile else None},'broker_order_state':'UNVERIFIED'}
...
def ensure_current_manifest(*,reason='CONSUMER_PRE_DISPATCH'):
 ...
 try:
  result=refresh(reason=reason,execution_context='LIVE_SCHEDULER',run_id=os.environ.get('ATLAS_RUN_ID'))
