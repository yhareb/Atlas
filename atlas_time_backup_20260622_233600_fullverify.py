...
def add_trading_days(start_day, days):
    cur = start_day
    added = 0
...
def trading_days_between(start_day, end_day):
    """Trading days from start_day to end_day, excluding start, including end if trading day."""
    if end_day < start_day:
        return -trading_days_between(end_day, start_day)
    cur = start_day
    days = 0
    while cur < end_day:
        cur += timedelta(days=1)
