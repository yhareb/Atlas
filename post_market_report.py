...
def get_handoff_close_performance():
...
def get_todays_buy_signals():
    """Get all BUY signals fired by the engine today from atlas.db."""
    conn = atlas_db.get_connection()
    cursor = conn.cursor()
    today = datetime.now().strftime('%Y-%m-%d')
    cursor.execute('''
        SELECT ticker, signal, score, rvol, entry_price
        FROM signals
        WHERE date(timestamp) = ?
        AND (signal LIKE '%BUY%')
        ORDER BY score DESC, rvol DESC
        LIMIT 8
    ''', (today,))
    ...
    lines = []
    for ticker, signal, score, rvol, entry in rows:
        signal_clean = signal.replace("🟢", "").replace("🟡", "").strip()
        lines.append(f"  • {ticker}  {signal_clean}  Score: {score}  RVOL: {rvol:.2f}  Entry: ${entry:.2f}")
    return lines
...
def generate_post_market_report():
    ...
    buy_lines, watch_lines = get_handoff_close_performance()
    if buy_lines:
    ...
    if buy_signals:
        lines.append("*Engine BUY Signals Today:*")
