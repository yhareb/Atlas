"""Production-grade provider quote normalization and deterministic resolution.

Read-only, credential-free adapter layer: callers supply provider-shaped response
objects. This module never reads credentials, performs network calls, calculates
advisory action, writes state, or imports protected alpha modules.
"""
...
SOURCE_PRIORITY={
 "MASSIVE_LAST_TRADE":("MASSIVE","PROVIDER_TRADE",0),
 "MASSIVE_SNAPSHOT":("MASSIVE","PROVIDER_SNAPSHOT",1),
 "MASSIVE_MINUTE_CLOSE":("MASSIVE","PROVIDER_MINUTE_CLOSE",2),
 "EODHD_REALTIME":("EODHD","PROVIDER_SNAPSHOT",3),
 "EODHD_DAILY_CLOSE":("EODHD","COMPLETED_DAILY_CLOSE",4),
}
...
def parse_massive_snapshot(ticker:str, raw:dict[str,Any])->dict[str,Any]:
 t=(raw or {}).get('ticker') or raw or {}; lt=t.get('lastTrade') or {}; lq=t.get('lastQuote') or {}; day=t.get('day') or {}; minute=t.get('min') or {}
...
def parse_massive_minute(ticker:str, raw:dict[str,Any])->dict[str,Any]:
 rows=(raw or {}).get('results') or []; r=rows[-1] if rows else {}
...
def normalize(candidates:list[dict[str,Any]],*,now:datetime,reference_prices:dict[str,float]|None=None,corporate_actions:dict[str,dict[str,Any]]|None=None)->dict[str,Any]:
 reference_prices=reference_prices or {}; corporate_actions=corporate_actions or {}; now=now.astimezone(timezone.utc); session=classify_session(now)['session']
 valid=[]; rejected=[]
 for raw in candidates or []:
  q=dict(raw); ticker=str(q.get('ticker') or '').upper(); route=str(q.get('provider_route') or '').upper(); reason=None; price=_num(q.get('price')); ts=parse_dt(q.get('timestamp'))
  if not ticker: reason='TICKER_MISSING'
  elif route not in SOURCE_PRIORITY: reason='PROVIDER_ROUTE_UNSUPPORTED'
  elif price is None: reason='PRICE_INVALID'
  elif not ts: reason='TIMESTAMP_MISSING_OR_MALFORMED'
  elif (ts-now).total_seconds()>30: reason='FUTURE_TIMESTAMP_REJECTED'
  elif route == 'EODHD_DAILY_CLOSE' and session != 'CLOSED': reason='DAILY_CLOSE_LIVE_SESSION_MISMATCH'
  elif route != 'EODHD_DAILY_CLOSE' and q.get('source_session') and q.get('source_session') != session: reason='WRONG_SESSION'
  elif (now-ts).total_seconds()>TTL.get(session,300) and q.get('source_class')!='COMPLETED_DAILY_CLOSE': reason='STALE_QUOTE'
  elif q.get('bid') not in (None,'') and q.get('ask') not in (None,'') and (_num(q.get('bid')) is None or _num(q.get('ask')) is None): reason='BID_ASK_MALFORMED'
  elif q.get('bid') not in (None,'') and q.get('ask') not in (None,'') and float(q['bid'])>float(q['ask']): reason='CROSSED_QUOTE_REJECTED'
  ca=corporate_actions.get(ticker) or {}
  if not reason and ca:
   if ca.get('requires_adjusted') and q.get('adjusted') is not True: reason='CORPORATE_ACTION_ADJUSTMENT_MISMATCH'
   elif ca.get('split_factor') not in (None,1,1.0) and q.get('split_factor') not in (ca.get('split_factor'),): reason='CORPORATE_ACTION_SPLIT_FACTOR_MISMATCH'
  ref=_num(reference_prices.get(ticker))
  if not reason and ref and abs(price-ref)/max(price,ref)>0.50 and not ca.get('verified_reference_reset'): reason='SANITY_BAND_REJECTED'
 ...
 # Same-session conflicts: conflict only among freshest route-best candidates within 60s; do not silently prefer disagreement >0.5%.
 by={}
 for q in valid: by.setdefault(q['ticker'],[]).append(q)
 selected={}
 for ticker,arr in by.items():
  arr.sort(key=lambda x:(x['provider_priority'],-parse_dt(x['timestamp']).timestamp()))
  top=arr[0]; peers=[x for x in arr if abs((parse_dt(x['timestamp'])-parse_dt(top['timestamp'])).total_seconds())<=60]
  conflict=[x for x in peers if abs(x['price']-top['price'])/max(x['price'],top['price'])>0.005]
  if conflict:
   for x in [top]+conflict:
    x['validity']='REJECTED'; x['rejection_reason']='SAME_SESSION_PROVIDER_DISAGREEMENT'; rejected.append(x)
   valid=[x for x in valid if x not in [top]+conflict]; selected[ticker]={"validity":"REJECTED","rejection_reason":"SAME_SESSION_PROVIDER_DISAGREEMENT","selected":None}
  else:selected[ticker]={"validity":"VALID","rejection_reason":None,"selected":top}
 # Core role selection uses only nonrejected candidates.
 role_selection={}
 for ticker,arr in by.items():
