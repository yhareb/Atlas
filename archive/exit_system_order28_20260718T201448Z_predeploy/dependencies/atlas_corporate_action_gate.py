...
class Authority:
    ...
    def decide(self,ticker,path="candidate",now=None):
        ...
        body={"version":1,"ticker":ticker,"path":path,"outcome":outcome,"reason":reason,"observed_at":observed.isoformat(),"evidence":evidence}; body["receipt_sha256"]=_sha(_canon(body))
        if _cycle_receipts:
            _cycle_receipts.record("corporate_action", {"candidate_id":ticker,"ticker":ticker,"security_identity":ticker,"path":path,"outcome":outcome,"reason":reason,"evidence_ids":[e.get("accession") for e in evidence if e.get("accession")],"source_times":[e.get("filing_date") for e in evidence if e.get("filing_date")],"freshness":"FRESH" if outcome!=DEFER else "DEFERRED","policy":"UNCHANGED_CLEAR_BLOCK_DEFER","final_write_choke":path=="final_automatic_trade_write","authority_receipt_sha256":body["receipt_sha256"]})
        return body
