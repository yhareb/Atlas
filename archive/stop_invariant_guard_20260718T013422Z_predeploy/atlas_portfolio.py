...
def _hours_between(earlier_str, later_dt):
    """Bounded helper: hours between a stored 'YYYY-MM-DD HH:MM:SS' string
    (assumed UTC, matching atlas_db._now()'s convention) and a datetime."""
    if not earlier_str:
        return None
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d %H:%M:%S.%f"):
        try:
            earlier_dt = datetime.strptime(earlier_str.split("+")[0], fmt).replace(tzinfo=timezone.utc)
...
# P0O-17: fill-time signal-freshness window (staging default, implementation
# parameter -- NOT a protected alpha constant; adjustable by Prof). Chosen
# conservatively based on the P0O-14 empirical finding that most signal
# decay was already visible well inside 24h of arming.
PULLBACK_FILL_MAX_SIGNAL_AGE_HOURS = 24.0


def _revalidate_pullback_fill(ticker, row, now=None):
    """
    P0O-17: pre-emptive fill-time revalidation gate for an armed pending
    pullback whose price trigger has just been touched. Re-queries the
    live `signals` table (via atlas_db.get_latest_signal, read-only) and
    compares against the stale arm-time snapshot already stored on `row`.

    Returns a dict: {"outcome": ..., "reason_code": ..., "fresh_signal":
    ..., "fresh_score": ..., "fresh_rvol_pass": ..., "age_hours": ...}

    outcome is one of: ALLOW_FILL, EXPIRE_STALE_SIGNAL, BLOCK_SIGNAL_DECAYED.

    This function does NOT call consider_buy(), does NOT mutate any DB
    row, and does NOT touch mechanical stop/exit logic, broker flow, or
    sizing/admission math -- it only decides whether the caller should
    proceed into the existing, unchanged consider_buy() pipeline this pass.
    Uses only already-computed pass/fail fields already stored in the
    `signals` table (e.g. the pre-computed pillar/volume pass-fail flags
    that are already visible in every rendered live report) -- it does not
    read, duplicate, or expose any protected scoring formula or constant.
    """
    now_dt = now or _now_utc()
    ticker = (ticker or "").upper()
    armed_at = row.get("armed_at")

    fresh = atlas_db.get_latest_signal(ticker)

    if not fresh:
        return {
            "outcome": "BLOCK_SIGNAL_DECAYED",
            "reason_code": "FILL_BLOCKED_LIVE_DATA_MISSING",
            "fresh_signal": None, "fresh_score": None, "fresh_rvol_pass": None,
            "age_hours": _hours_between(armed_at, now_dt),
        }

    fresh_ts = fresh.get("timestamp")
    age_hours = _hours_between(fresh_ts, now_dt)
    if age_hours is None:
        age_hours = _hours_between(armed_at, now_dt)

    if age_hours is not None and age_hours > PULLBACK_FILL_MAX_SIGNAL_AGE_HOURS:
        return {
            "outcome": "EXPIRE_STALE_SIGNAL",
            "reason_code": "FILL_EXPIRED_SIGNAL_AGE_EXCEEDED",
            "fresh_signal": fresh.get("signal"), "fresh_score": fresh.get("score"),
            "fresh_rvol_pass": None, "age_hours": age_hours,
        }

    fresh_label = str(fresh.get("signal") or "")
    if "AVOID" in fresh_label:
        return {
            "outcome": "EXPIRE_STALE_SIGNAL",
            "reason_code": "FILL_EXPIRED_SIGNAL_AVOID",
            "fresh_signal": fresh_label, "fresh_score": fresh.get("score"),
            "fresh_rvol_pass": None, "age_hours": age_hours,
        }

    is_buy_tier = "BUY" in fresh_label
    try:
        fresh_pillars = int(str(fresh.get("score") or "0").split("/")[0])
    except Exception:
        fresh_pillars = 0
    fresh_volume_field = str(fresh.get("volume") or "")
    fresh_rvol_pass = fresh_volume_field.strip().startswith("\u2705")  # \u2705 = existing engine-computed pass marker (already public in reports)

    if not is_buy_tier or fresh_pillars < 3:
        return {
            "outcome": "BLOCK_SIGNAL_DECAYED",
            "reason_code": "FILL_BLOCKED_SIGNAL_DECAYED_SCORE",
            "fresh_signal": fresh_label, "fresh_score": fresh.get("score"),
            "fresh_rvol_pass": fresh_rvol_pass, "age_hours": age_hours,
        }

    if not fresh_rvol_pass:
        return {
            "outcome": "BLOCK_SIGNAL_DECAYED",
            "reason_code": "FILL_BLOCKED_SIGNAL_DECAYED_RVOL",
            "fresh_signal": fresh_label, "fresh_score": fresh.get("score"),
            "fresh_rvol_pass": fresh_rvol_pass, "age_hours": age_hours,
        }
    ...
    return {
        "outcome": "ALLOW_FILL",
        "reason_code": "FILL_REVALIDATION_PASSED",
        "fresh_signal": fresh_label, "fresh_score": fresh.get("score"),
        "fresh_rvol_pass": fresh_rvol_pass, "age_hours": age_hours,
    }
...
def evaluate_pending_pullback(ticker, dry_run=True, regime=None, pending=None, reserved_cash=0.0):
    ...
    if state["last_close"] <= trigger:
        # P0O-17: pre-emptive fill-time revalidation gate. Inserted strictly
        # between the existing price-trigger check (above, unchanged) and
        # the existing consider_buy() call (below, unchanged). Does not
        # touch mechanical stop/exit logic, broker flow, or sizing/admission
        # math -- it only decides whether to proceed into consider_buy()
        # this pass, or to WAIT/EXPIRE the armed pullback instead.
        revalidation = _revalidate_pullback_fill(ticker, row, now=_now_utc())
        if revalidation["outcome"] == "EXPIRE_STALE_SIGNAL":
            atlas_db.expire_pending_pullback(ticker)
            return {
                "ticker": ticker, "action": "EXPIRE", "score": row.get("score"),
                "reason": (f"PULLBACK EXPIRED — {ticker}, live signal decayed "
                           f"({revalidation['reason_code']}); fresh_signal="
                           f"{revalidation['fresh_signal']}, fresh_score="
                           f"{revalidation['fresh_score']}, age_hours="
                           f"{round(revalidation['age_hours'], 1) if revalidation['age_hours'] is not None else None}."),
                "pending_id": row.get("id"),
                "reason_code": revalidation["reason_code"],
            }
        if revalidation["outcome"] == "BLOCK_SIGNAL_DECAYED":
            return {
                "ticker": ticker, "action": "WAIT", "score": row.get("score"),
                "reason": (f"PULLBACK TRIGGER TOUCHED BUT BLOCKED — {ticker}, "
                           f"live signal no longer supports fill "
                           f"({revalidation['reason_code']}); fresh_signal="
                           f"{revalidation['fresh_signal']}, fresh_score="
                           f"{revalidation['fresh_score']}."),
                "pending_id": row.get("id"), "wait_type": "PULLBACK_WAITING",
                "entry": round(trigger, 2), "expires_at": row.get("expires_at"),
                "reason_code": revalidation["reason_code"],
                "fill_revalidation": revalidation,
            }
        # outcome == ALLOW_FILL -> proceed into the existing, unchanged
        # consider_buy() pipeline exactly as before this patch.
        sig = row.get("signal_result") or {}
        ...
        decision["fill_revalidation"] = revalidation
