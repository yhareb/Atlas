...
def _perme_macro_prose(briefing: str, context: dict[str, Any] | None, now_et: datetime, flags: list[str]) -> str:
    ...
    regime = _regime_label(briefing, flags)
    headline = "Wall Street tone is cautious" if regime == "RISK-OFF" else "Wall Street tone is constructive" if regime == "RISK-ON" else "Wall Street tone is mixed"
    ...
    lines = [
        f"📰 Breaking headline — {_scan_line('📉', headline + ': ' + driver)}",
        f"📉 Market tape — {_scan_line('💻' if any(term in tape.upper() for term in ('NASDAQ', 'FUTURES', 'TECH')) else _readability_emoji(tape, '📉'), tape)}",
        f"🔥 What drove it — {_scan_line('🛢️' if any(term in driver.upper() for term in ('IRAN', 'HORMUZ', 'OIL', 'MIDDLE EAST')) else _readability_emoji(driver, '⚠️'), driver)}",
    ]
    geopolitical = _geopolitical_market_impact(evidence, flags)
    if geopolitical:
        lines.append(f"🌍 Geopolitical market impact — {_scan_line('⚠️', geopolitical)}")
    lines.extend([
        f"🔄 Sector rotation — {_scan_line(_readability_emoji(rotation, '🔄'), rotation)}",
        f"💼 Portfolio impact — {_scan_line('💼', portfolio)}",
        f"🧠 Perme view — {_scan_line('🏦' if any(term in perme_view.upper() for term in ('RATE', 'FED', 'YIELD')) else _readability_emoji(perme_view, '⚠️'), perme_view)}",
        f"📅 Next catalyst — {_scan_line('⏳', catalyst)}",
        "⚖️ Context vs TFE action — ✅ Perme is market intelligence only. TFE remains deterministic; Atlas does not change trading rules, routing, or risk from this prose.",
    ])
    return "\n\n".join(lines)
