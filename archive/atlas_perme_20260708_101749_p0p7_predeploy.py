...
def _perme_macro_prose(briefing: str, context: dict[str, Any] | None, now_et: datetime, flags: list[str]) -> str:
    ...
    lines = [
        f"📰 Breaking headline — {headline}: {driver}",
        f"📉 Market tape — {_market_tape_from_evidence(evidence)}",
        f"🔥 What drove it — {driver}",
    ]
    geopolitical = _geopolitical_market_impact(evidence, flags)
    if geopolitical:
        lines.append(f"🌍 Geopolitical market impact — {geopolitical}")
    lines.extend([
        f"🔄 Sector rotation — {_rotation_from_evidence(evidence, flags)}",
        f"💼 Portfolio impact — {_portfolio_impact_from_flags(flags)}",
        f"🧠 Perme view — {_perme_view_from_regime(briefing, flags)}",
        f"📅 Next catalyst — {_next_catalyst_from_brief(context, now_et, evidence, risks)}",
        "⚖️ Context vs TFE action — Perme is market intelligence only. TFE remains deterministic; Atlas does not change entries, exits, stops, targets, routing, or risk from this prose.",
    ])
    return "\n\n".join(lines)
