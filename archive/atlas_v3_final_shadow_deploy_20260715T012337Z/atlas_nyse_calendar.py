"""Production-grade deterministic NYSE calendar/session adapter.

Staging-only standard-library implementation. Rules cover regular NYSE full
holidays, observed holidays, Good Friday, early closes, DST through zoneinfo,
and completed-session retention calculations. No production imports or writes.
"""
...
def full_holidays(year:int)->set[date]:
    ...
    # An observed next-year New Year's Day may fall in current calendar year.
    next_obs=_observed(date(year+1,1,1))
    if next_obs.year==year: holidays.add(next_obs)
    return holidays
...
def early_close_days(year:int)->set[date]:
    ...
    thanksgiving=_nth_weekday(year,11,3,4); fri=thanksgiving+timedelta(days=1)
    if is_trading_day(fri): out.add(fri)
    # Christmas Eve when a weekday and a trading day.
    xmas_eve=date(year,12,24)
    if is_trading_day(xmas_eve): out.add(xmas_eve)
    # Session before Independence Day when it is the customary weekday early close.
    july4=date(year,7,4)
    candidate=july4-timedelta(days=1)
    while not is_trading_day(candidate): candidate-=timedelta(days=1)
    if candidate.weekday()<5 and (july4-candidate).days<=3: out.add(candidate)
    return out
...
def session_schedule(d:date)->dict:
    ...
    op=datetime.combine(d,time(9,30),ET); cl=datetime.combine(d,close_t,ET)
...
def classify_session(now:datetime)->dict:
    ...
    et=now.astimezone(ET); sched=session_schedule(et.date())
    if not sched["is_trading_day"]: state="CLOSED"
    else:
        t=et.timetz().replace(tzinfo=None); close_t=datetime.fromisoformat(sched["close"]).timetz().replace(tzinfo=None)
        if t < time(4,0): state="CLOSED"
        elif t < time(9,30): state="PRE_MARKET"
        elif t < close_t: state="REGULAR"
        elif t < time(20,0): state="AFTER_MARKET"
        else: state="CLOSED"
...
def latest_completed_session(now:datetime)->str:
    now=now if now.tzinfo else now.replace(tzinfo=timezone.utc); et=now.astimezone(ET); d=et.date()
    sched=session_schedule(d)
    if not sched["is_trading_day"] or now < datetime.fromisoformat(sched["close"]).astimezone(timezone.utc): d-=timedelta(days=1)
    while not is_trading_day(d): d-=timedelta(days=1)
    return d.isoformat()


def completed_session_expiry(since:datetime, sessions:int=2)->str:
    since=since if since.tzinfo else since.replace(tzinfo=timezone.utc); d=since.astimezone(ET).date(); n=0
    while n<sessions:
        d+=timedelta(days=1)
        if is_trading_day(d): n+=1
...
def session_schedule_governed(d:date, *, override_authority:dict|None=None)->dict:
    base=session_schedule(d); auth=override_authority or {"authority_state":"RULE_DEFAULT","override":None,"calendar_blocker":False}
    ...
    out=dict(base);out.update({"authority_state":auth.get("authority_state","RULE_DEFAULT"),"calendar_blocker":False,"override_digest":(ov or {}).get("override_digest")})
    if ov and ov.get("closure_type")=="FULL_CLOSE":
        ...
        close_t=time.fromisoformat(ov["early_close_time_et"]);op=datetime.combine(d,time(9,30),ET);cl=datetime.combine(d,close_t,ET)
...
def completed_session_expiry_governed(since:datetime,sessions:int=2,*,authority_lookup=None)->str:
    since=since if since.tzinfo else since.replace(tzinfo=timezone.utc);d=since.astimezone(ET).date();n=0
    while n<sessions:
        d+=timedelta(days=1);auth=authority_lookup(d) if authority_lookup else None;s=session_schedule_governed(d,override_authority=auth)
        if s.get("calendar_blocker"):raise ValueError("CALENDAR_AUTHORITY_BLOCKER")
        if s.get("is_trading_day"):n+=1
