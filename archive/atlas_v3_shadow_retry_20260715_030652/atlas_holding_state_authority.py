...
"""Atlas Holding-State Authority staged core (Phase 2A/2B).

Pure deterministic staged implementation. Does not import production Atlas modules,
does not write atlas.db, does not send Telegram, does not touch broker state.
"""
...
from atlas_holding_state_schema import (
    SCHEMA_VERSION, QUOTE_SELECTION_VERSION, LEDGER_SCHEMA_VERSION, ACTION_RANK,
    STRONG_ACTIONS, REASON_CODES, LEDGER_EVENT_TYPES, PriceRole, QuoteSelection,
    AxisState, AdvisoryAction, BrokerLifecycle, RetentionState, validate_packet_shape,
)
...
REASON_ALIASES = {
    "STOP BREACHED REGULAR": "REGULAR_STOP_BREACH", "STOP_BREACHED_REGULAR": "REGULAR_STOP_BREACH",
    "STOP BREACHED PRE MARKET": "PREMARKET_STOP_BREACH", "STOP_BREACHED_PRE_MARKET": "PREMARKET_STOP_BREACH",
    "STOP BREACHED AFTER MARKET": "AFTERHOURS_STOP_BREACH", "PROTECT PROFIT": "PROFIT_GIVEBACK",
    "TIGHTEN": "PROFIT_GIVEBACK", "PROFIT PROTECTION": "PROFIT_GIVEBACK",
    "TRIM": "PROFIT_PROTECTION_TRIM_REVIEW", "EXIT": "THESIS_DETERIORATION",
    "THESIS WEAK": "THESIS_DETERIORATION", "THESIS BROKEN": "THESIS_BROKEN",
    "CATALYST": "CATALYST_RISK", "SECTOR": "SECTOR_BREAKDOWN", "REGIME": "REGIME_RISK",
}
ACTION_ALIASES = {
    "SELL": "SELL NOW", "SELL_NOW": "SELL NOW", "EXIT": "EXIT REVIEW", "EXIT_REVIEW": "EXIT REVIEW",
    "TRIM": "TRIM REVIEW", "TRIM_REVIEW": "TRIM REVIEW", "HOLD_TIGHT": "HOLD TIGHT",
    "WATCH CLOSELY": "HOLD TIGHT", "PROTECT PROFIT": "TRIM REVIEW", "TIGHTEN": "HOLD TIGHT",
    "MISSING": "DATA INCOMPLETE", "STALE": "DATA INCOMPLETE", "": "DATA INCOMPLETE", None: "DATA INCOMPLETE",
}
...
def normalize_reason_codes(reasons: Iterable[Any] | Any) -> tuple[str, ...]:
    ...
    if isinstance(reasons, (str, bytes)):
        ...
        try: items = list(reasons)
        except TypeError: items = [reasons]
    out: list[str] = []
    for r in items:
        ...
        if code not in REASON_CODES and code not in {"EXPLICIT_STOP_CLEAR", "EXPLICIT_PROFIT_CLEAR", "EXPLICIT_THESIS_CLEAR"}:
            # Keep deterministic but namespaced unknowns; tests can still prove presence.
...
def _sort_key(q: dict[str, Any], role: str) -> tuple[int, float]:
    ...
    pref = {
        "PERSISTED_STOP_EVENT": 0 if role == "stop" else 99,
        "PROVIDER_TRADE": 1,
        "PROVIDER_MINUTE_CLOSE": 2,
        "PROVIDER_AGGREGATE": 3,
        "PROVIDER_SNAPSHOT": 4,
        "COMPLETED_DAILY_CLOSE": 5,
        "CACHE": 6,
        "FALLBACK_DISPLAY_ONLY": 90,
        "REFERENCE": 91,
        "ENTRY_REFERENCE": 92,
    }.get(cls, 80)
...
def select_quotes(quotes: list[dict[str, Any]], *, report_session: str, now: datetime, prior_event_timestamp: Any = None) -> dict[str, Any]:
    ...
    for q in quotes or []:
        ...
        if q.get("rejection_reason"):
            rejected.append(_quote_reject(q, str(q.get("rejection_reason")))); continue
        if price is None:
            rejected.append(_quote_reject(q, "PRICE_INVALID")); continue
        if q.get("bid") not in (None, "") and q.get("ask") not in (None, ""):
            try:
                if float(q["bid"]) > float(q["ask"]):
                    rejected.append(_quote_reject(q, "CROSSED_QUOTE_REJECTED")); continue
            except Exception:
                rejected.append(_quote_reject(q, "BID_ASK_MALFORMED")); continue
        if cls not in DISPLAY_SOURCE_CLASSES:
            rejected.append(_quote_reject(q, "SOURCE_CLASS_INVALID")); continue
        if cls not in {"FALLBACK_DISPLAY_ONLY", "REFERENCE", "ENTRY_REFERENCE"}:
            if not ts:
                rejected.append(_quote_reject(q, "TIMESTAMP_MISSING_OR_MALFORMED")); continue
            if (ts - now).total_seconds() > 30:
                rejected.append(_quote_reject(q, "FUTURE_TIMESTAMP_REJECTED")); continue
        # display: all sane prices except persisted stop event unless explicitly requested as event display
        if cls != "PERSISTED_STOP_EVENT" and _session_ok(qsession, report_session, "display", cls):
            candidates["display"].append(q)
        # valuation: only valid valuation classes, fresh/session ok, not fallback
        ...
        # stop eval: provider/event only; fallback/cache no; daily close only for CLOSED/PP via component not live stop
        if cls in STOP_EVAL_SOURCE_CLASSES:
            if cls == "PERSISTED_STOP_EVENT":
                # Persisted stop evidence must be explicit; it cannot silently become a current market quote.
                if not q.get("event_id"):
                    rejected.append(_quote_reject(q, "STOP_EVENT_ID_MISSING")); continue
                if q.get("trade_id") in (None, ""):
                    rejected.append(_quote_reject(q, "STOP_EVENT_TRADE_ID_MISSING")); continue
                if q.get("event_stop") in (None, "") and q.get("stop") in (None, ""):
                    rejected.append(_quote_reject(q, "STOP_EVENT_STOP_MISSING")); continue
                if ts:
                ...
                if prior_ts and ts <= prior_ts and q.get("clears_prior_event"):
                    rejected.append(_quote_reject(q, "OLDER_THAN_PRIOR_EVENT_CANNOT_CLEAR")); continue
                candidates["stop"].append(q); valid_for_conflict.append(q)
    # Conflict: same timestamp/session/source priority with materially different prices -> reject stop eval.
    conflict = False
    ...
    def role(q: Optional[dict[str, Any]], name: str) -> dict[str, Any]:
        ...
        if cls == "PERSISTED_STOP_EVENT":
            out.update({
                ...
                "trade_id": q.get("trade_id"),
                "lot_id": q.get("lot_id"),
                "ticker": q.get("ticker"),
                "event_timestamp": q.get("timestamp"),
                "event_price": q.get("price"),
                "event_stop": q.get("event_stop") if q.get("event_stop") not in (None, "") else q.get("stop"),
                "validity_state": q.get("validity_state") or "ACTIVE_STOP_BREACH",
                "supersession_proof": q.get("supersession_proof") or "NO_SUPERSEDING_EVENT_IN_FIXTURE",
                "source_digest": sha_json({k:q.get(k) for k in ("event_id","trade_id","lot_id","ticker","timestamp","price","event_stop","stop","source_class")}),
            })
        return out
    ...
    return {
        "selection_policy_version": QUOTE_SELECTION_VERSION,
        "report_session": report_session,
        "selected_display_price": role(d, "display"),
        "selected_valuation_price": role(v, "valuation"),
        "selected_stop_evaluation_price": role(s, "stop_eval"),
        "rejected_quotes": rejected,
        "timestamp_ordering_proof": proof,
    }
...
def can_clear(prior: dict[str, Any] | None, clearing: dict[str, Any] | None, *, now: datetime) -> tuple[bool, Optional[dict[str, Any]], str]:
    ...
    cleared_by = {
        "source": clearing.get("source"),
        "timestamp": iso(ts),
        "digest": clearing.get("digest") or sha_json(clearing),
        "reason_code": sorted(clear_reasons)[0] if clear_reasons else "UNKNOWN",
        "human_reason": clearing.get("human_reason") or "explicit reason-based clearing evidence accepted",
    }
...
def build_holding_state(*, trade: dict[str, Any], quotes: list[dict[str, Any]], report_session: str, now: datetime, daily: dict[str, Any] | None = None, profit_protection: dict[str, Any] | None = None, broker: dict[str, Any] | None = None, prior_action: dict[str, Any] | None = None, clearing_evidence: dict[str, Any] | None = None, ledger: "ActionLedger" | None = None) -> dict[str, Any]:
    ...
    if stop_price is not None and stop is not None:
        ...
        if sp <= stop:
            if report_session == "REGULAR":
                observed_state = "STOP_BREACHED_REGULAR"; obs_reasons = ("REGULAR_STOP_BREACH",); stop_action = "SELL NOW"
            elif report_session == "PRE_MARKET":
                observed_state = "STOP_BREACHED_PRE_MARKET"; obs_reasons = ("PREMARKET_STOP_BREACH",); stop_action = "EXIT REVIEW"
            elif report_session == "AFTER_MARKET":
                observed_state = "STOP_BREACHED_AFTER_MARKET"; obs_reasons = ("AFTERHOURS_STOP_BREACH",); stop_action = "EXIT REVIEW"
            else:
                observed_state = "STOP_BREACHED_REGULAR"; obs_reasons = ("REGULAR_STOP_BREACH",); stop_action = "SELL NOW"
        elif sp <= stop * 1.015:
            observed_state = "NEAR_STOP"; obs_reasons = ()
        else:
            observed_state = "ABOVE_STOP"; obs_reasons = ()
    elif qs["rejected_quotes"]:
        observed_state = "INVALID_OR_STALE_EVENT"; obs_reasons = ("DATA_STALE_OR_MISSING",)
    ...
    axes = {
        "observed_market_risk_state": asdict(AxisState(observed_state, current_reasons, None, iso(now))),
        "advisory_action": asdict(AdvisoryAction(current_action, current_reasons, "CANONICAL_PRECEDENCE_ENGINE", None)),
        "broker_ledger_lifecycle": asdict(broker_lifecycle),
    }
    stop_eval = qs.get("selected_stop_evaluation_price") or {}
    components = {
        "daily_reunderwriting": {"action": daily_action, "freshness": daily_fresh, "reason_codes": daily_reasons, "digest": sha_json(daily or {})},
        "profit_protection": {"action": pp_action, "freshness": pp_fresh, "reason_codes": pp_reasons, "digest": sha_json(profit_protection or {})},
        "stop_lifecycle": {
            "state": observed_state,
            "event_id": stop_eval.get("event_id"),
            "trade_id": stop_eval.get("trade_id"),
            "lot_id": stop_eval.get("lot_id"),
            "ticker": stop_eval.get("ticker"),
            "event_timestamp": stop_eval.get("event_timestamp"),
            "event_price": stop_eval.get("event_price"),
            "event_stop": stop_eval.get("event_stop"),
            "validity_state": stop_eval.get("validity_state") or ("ACTIVE_STOP_BREACH" if stop_eval.get("source") == "PERSISTED_STOP_EVENT" and observed_state.startswith("STOP_BREACHED") else None),
            "supersession_proof": stop_eval.get("supersession_proof"),
            "source_digest": stop_eval.get("source_digest"),
            "validity_reason": ";".join(obs_reasons) or "NO_ACTIVE_STOP_EVENT",
        },
    }
    ...
    final_precedence = {
        "selected_advisory_action": current_action,
        "selected_observed_state": observed_state,
        "selected_broker_lifecycle_state": broker_lifecycle.state,
        "precedence_reason": f"separate axes; advisory={current_action}; observed={observed_state}; broker={broker_lifecycle.state}",
        "component_freshness": {"quote": qs["selected_stop_evaluation_price"].get("validity"), "daily": daily_fresh, "profit_protection": pp_fresh, "broker": broker_lifecycle.state},
    }
    packet = {
        ...
        "clearing": {"clearance_status": "CLEARED" if cleared_by else ("EXPIRED_TO_DATA_INCOMPLETE" if ledger_event_type == "SOURCE_SUPERSEDED" else "NOT_CLEARED"), "cleared_by": cleared_by},
        "components": components,
        "final_precedence": final_precedence,
        "ledger": {"latest_event_id": None, "events_applied": [], "idempotency_key": None},
        "authority": {"report_local_authority_allowed": False, "broker_authority": "NO", "automatic_trade_closure": "NO"},
        "digests": {"canonical_input_digest": input_digest, "packet_digest": None},
    }
...
class ActionLedger:
    ...
    def event_identity(*, trade_identity: dict[str, Any], event_type: str, observed_state: str, advisory_action: str, broker_lifecycle_state: str, reason_codes: Iterable[str], source_type: str, source_timestamp: str, source_digest: str, prior_event_id: Any = None, clearing_reason: Any = None) -> str:
        obj = {
            ...
            "trade_id": str(trade_identity.get("trade_id") or ""),
            "ticker": str(trade_identity.get("ticker") or "").upper(),
            "event_type": event_type,
            "action_axis": "ADVISORY",
            "observed_state": observed_state,
            "advisory_action": advisory_action,
            "broker_lifecycle_state": broker_lifecycle_state,
            "reason_codes": sorted(normalize_reason_codes(reason_codes)),
            "source_type": source_type,
            "source_timestamp": source_timestamp,
            "source_digest": source_digest,
            "prior_event_id": prior_event_id or "",
            "clearing_reason": clearing_reason or "",
        }
        return hashlib.sha256(stable_json(obj).encode()).hexdigest()
    def append_event(self, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            if row:
                con.commit(); return {"status": "IDEMPOTENT_NO_OP", "event_id": event_id}
    ...
    def count(self) -> int:
        con=self._connect()
        try: return int(con.execute("SELECT COUNT(*) FROM action_ledger_events").fetchone()[0])
        finally: con.close()
    def simulate_partial_failure(self, first_kwargs: dict[str, Any], second_kwargs: dict[str, Any]) -> None:
        con = self._connect()
        try:
            con.execute("BEGIN IMMEDIATE")
            # Deliberately insert first then fail; caller verifies rollback leaves zero delta.
