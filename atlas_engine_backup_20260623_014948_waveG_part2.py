...
def check_news_sentiment(ticker):
    ...
    try:
        sr = requests.get("https://eodhd.com/api/sentiments", params={"api_token": EODHD_API_KEY, "fmt": "json", "s": f"{ticker}.US"}, timeout=10)
        ...
        nr = requests.get("https://eodhd.com/api/news", params={"api_token": EODHD_API_KEY, "fmt": "json", "s": f"{ticker}.US", "limit": 3}, timeout=10)
...
def check_eodhd_atr(ticker):
    ...
    try:
        ...
        r=requests.get(f"https://eodhd.com/api/technical/{ticker}.US", params={"api_token":EODHD_API_KEY,"fmt":"json","function":"atr","period":14,"from":start,"to":today}, timeout=10)
        if r.status_code==200:
            rows=r.json()
...
def check_massive_financials(ticker):
    ...
    try:
        r=requests.get(f"{MASSIVE_BASE}/vX/reference/financials", params={"apiKey":MASSIVE_API_KEY,"ticker":ticker,"timeframe":"ttm","limit":1}, timeout=10)
        if r.status_code==200:
...
def analyze_ticker(ticker, regime=None):
    ...
    # Fundamentals / indicators / ATR / sentiment are labels only, only for 3/4 or 4/4 candidates.
