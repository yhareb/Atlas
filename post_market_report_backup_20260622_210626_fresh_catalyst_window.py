...
def get_postmarket_catalyst_universe(market_date=None, limit=40):
    ...
    try:
        today = (market_date or datetime.now(ZoneInfo("America/New_York")).date()).strftime('%Y-%m-%d')
...
def get_engine_catalyst_watchlist(market_date=None, limit=40, max_hits=12):
    ...
    for ticker in tickers:
        ...
        try:
            ...
            print(f"[post-market] catalyst check failed for {ticker}: {e}")
            continue
        if hit:
...
def generate_post_market_report(send=True):
    ...
    catalyst_lines, checked = get_engine_catalyst_watchlist(today)
