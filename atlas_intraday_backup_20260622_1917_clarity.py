import os, sys, datetime, contextlib, io
    ...
    if high:
        for h in high:
            ...
            if action == "WAIT" and reason:
                action_text = f"WAIT ({reason})"
            elif reason:
                action_text = f"{action} ({reason})"
            else:
                action_text = action or "n/a"
            lines.append(f"{h.get('ticker','?')} — {h.get('score','?')} — {action_text}")
    else:
        lines.append("none")
