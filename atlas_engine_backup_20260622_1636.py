...
def analyze_ticker(ticker, regime=None):
    ...
    # --- NEW LIQUIDITY/PRICE FLOOR ---
    # Reject penny stocks (under $5) and illiquid junk immediately
    current_price = data['close'].iloc[-1]
    if current_price < 5.0:
        return {
            "ticker": ticker, "score": 0, "action": "AVOID",
            "pillars": {"Trend": False, "Momentum": False, "Catalyst": False, "Insider": False},
            "rationale": f"Price {current_price:.2f} < $5.00 floor",
            "current_price": current_price, "stop_loss": 0.0, "risk_amount": 0.0, "shares": 0
        }
    if 'ATR' not in data.columns or data['ATR'].isna().all() or data['ATR'].iloc[-1] == 0:
        return {
            "ticker": ticker, "score": 0, "action": "AVOID",
            "pillars": {"Trend": False, "Momentum": False, "Catalyst": False, "Insider": False},
            "rationale": "Insufficient data for ATR/liquidity",
            "current_price": current_price, "stop_loss": 0.0, "risk_amount": 0.0, "shares": 0
        }
    # ---------------------------------
