...
def discover_tickers():
    ...
    if EODHD_API_KEY:
        try:
            ...
            sr = requests.get(
                ...
                params={"api_token": EODHD_API_KEY, "fmt": "json", "filters": _json.dumps(filters), "limit": 20, "sort": "refund_1d_p.desc"},
                headers={"Accept": "application/json"}, timeout=10,
            )
            if sr.status_code == 200:
                for row in (sr.json().get("data") or [])[:20]:
    ...
    return ordered[:25]   # movers guaranteed in; cap raised slightly to 25
