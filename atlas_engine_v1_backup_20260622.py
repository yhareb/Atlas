...
def get_massive_aggs(ticker, days=365 ):
...
def analyze_ticker(ticker):
    ...
    highs_50 = max([day['h'] for day in aggs[-50:]]) if len(aggs) >= 50 else current_price
    if current_price >= (highs_50 * 0.90):
        pillars_met += 1
        pillar_details.append("✅ Relative Strength: YES (Within 10% of 50D High)")
    else:
        pillar_details.append("❌ Relative Strength: NO")

    # Pillar 3: Volume
    if rvol >= 1.2:
        pillars_met += 1
        pillar_details.append(f"✅ Volume: YES (RVOL: {rvol:.2f})")
    else:
        pillar_details.append(f"❌ Volume: NO (RVOL: {rvol:.2f})")

    # Pillar 4: Catalyst (News + Analyst Upgrade)
